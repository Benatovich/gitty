...
module.exports = class QuoteService {
  static async generate() {
    ...
    // const quoteOne = await Quote.insert({ author: dataOne[0].author, content: dataOne[0].en });
    const quoteTwo = await Quote.insert({ author: dataTwo[0].character, content: dataTwo[0].quote });
    // const quoteThree = await Quote.insert({ author: dataThree[0].author, content: dataThree[0].content });
    
    // console.log('||quoteOne', quoteOne);
    console.log('||quoteTwo', quoteTwo);
    // console.log('||quoteThree', quoteThree);
